...
/**
* NOTE:  This doesn't work according to EC guidelines.
* The <code>OptimalSimulator</code> class only has one method called .simulate() that
...
import java.util.*;  //Used for the Extra Credit for a list of Queues
import java.util.Vector;

public class OptimalSimulator extends Vector<Request>{
  ...
  **/
  ...
  throws InvalidFloorsException, InvalidLengthException, InvalidElevatorsException, InvalidProbabilityException{
    ...
    // NOTE:  For Extra Credit: Initializes boolean array for elevator
    // directions
    boolean[] elevatorDirections = new boolean[numberOfElevators];

    //NOTE:  For Extra Credit:  Initializes all directions for the elevator
    //array as up (true)
    for (boolean direction : elevatorDirections){
      direction = true;
    }

    //NOTE:  For Extra Credit:  Creates a Request Vector for unread Requests
    Vector<Request> extraRequests = new Vector<Request>(10,1);
    ...
    while (timer > 0){
      ...
      for (Elevator elevator : elevatorArray){
        ...
        if (elevator.getCurrentState() == elevator.IDLE){
          ...
          try{
            //Dequeues the queue if there is something in the queue
            if (!elevatorQueue.isEmpty()){
              elevator.setRequest(elevatorQueue.dequeue());

              //NOTE:  For extra Credit
              //If the elevator's new request is equal to
              if (elevator.getRequest().getSource() < elevator.getRequest().getDest()){
                elevatorDirections[elevatorQueue.indexOf(elevator)] = true;
              }else elevatorDirections[elevatorQueue.indexOf(elevator)] = false;
